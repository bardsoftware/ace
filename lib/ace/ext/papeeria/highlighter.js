  ...
  define(function(require, exports, module) {
    ...
    highlightBrackets = function(editor, pos) {
      ...
      if (!pos.mismatch) {
        ...
        if (pos.left && !pos.right) {
          rangeLeft = new Range(pos.left.row, pos.left.column, 2e308, 2e308);
    ...
    findSurroundingBrackets = function(session, pos) {
      ...
      allBrackets = {
        left: [session.$findOpeningBracketWithIterator('}', pos, newFilteringIterator('{', '}', session, pos, false)), session.$findOpeningBracketWithIterator(']', pos, newFilteringIterator('[', ']', session, pos, false)), session.$findOpeningBracketWithIterator(')', pos, newFilteringIterator('(', ')', session, pos, false))],
        right: [session.$findClosingBracketWithIterator('{', pos, newFilteringIterator('{', '}', session, pos, true)), session.$findClosingBracketWithIterator('[', pos, newFilteringIterator('[', ']', session, pos, true)), session.$findClosingBracketWithIterator('(', pos, newFilteringIterator('(', ')', session, pos, true))]
      };
      leftNearest = null;
      rightNearest = null;
      key = 0;
