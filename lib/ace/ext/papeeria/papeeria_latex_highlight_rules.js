...
(function() {
  ...
  define(function(require, exports, module) {
    "use strict";
    var EQUATION_ASTERISK_REGEX, EQUATION_ASTERISK_STATE, EQUATION_REGULAR_REGEX, EQUATION_REGULAR_STATE, EQUATION_STATE, EQUATION_TOKENTYPE, LIST_ENUMERATE_REGEX, LIST_ENUMERATE_STATE, LIST_ITEMIZE_REGEX, LIST_ITEMIZE_STATE, LIST_STATE, LIST_TOKENTYPE, MATH_ALTERNATIVE_CLOSING_REGEX, MATH_ALTERNATIVE_OPENING_REGEX, MATH_ALTERNATIVE_STATE, MATH_INLINE_CLOSING_REGEX, MATH_INLINE_OPENING_REGEX, MATH_INLINE_STATE, MATH_MULTILINE_CLOSING_REGEX, MATH_MULTILINE_OPENING_REGEX, MATH_MULTILINE_STATE, PapeeriaLatexHighlightRules, START_STATE, TextHighlightRules, oop;
    ...
    PapeeriaLatexHighlightRules = function() {
       ...
       */
      var basicRules, beginRule, citationsRules, endRule, equationStartRules, listStartRules, mathEndRules, mathStartRule, popState, pushState, specificTokenForState, state;
      ...
      equationStartRules = [beginRule(EQUATION_REGULAR_REGEX, EQUATION_REGULAR_STATE), beginRule(EQUATION_ASTERISK_REGEX, EQUATION_ASTERISK_STATE), mathStartRule(MATH_MULTILINE_OPENING_REGEX, MATH_MULTILINE_STATE), mathStartRule(MATH_INLINE_OPENING_REGEX, MATH_INLINE_STATE), mathStartRule(MATH_ALTERNATIVE_OPENING_REGEX, MATH_ALTERNATIVE_STATE)];
      citationsRules = [
        {
      ...
      this.$rules[LIST_ITEMIZE_STATE] = [].concat(equationStartRules, listStartRules, citationsRules, [
        endRule(LIST_ITEMIZE_REGEX), {
          token: ["storage.type", "lparen", "variable.parameter", "rparen"],
          regex: "(\\\\(?:begin|end))({)(\\w*)(})"
        }
      ]);
      this.$rules[LIST_ENUMERATE_STATE] = [].concat(equationStartRules, listStartRules, citationsRules, [
        endRule(LIST_ENUMERATE_REGEX), {
          token: ["storage.type", "lparen", "variable.parameter", "rparen"],
          regex: "(\\\\(?:begin|end))({)(\\w*)(})"
        }
      ]);
      this.$rules[EQUATION_REGULAR_STATE] = [endRule(EQUATION_REGULAR_REGEX)];
      this.$rules[EQUATION_ASTERISK_STATE] = [endRule(EQUATION_ASTERISK_REGEX)];
      this.$rules[MATH_INLINE_STATE] = mathEndRules(MATH_INLINE_CLOSING_REGEX);
      this.$rules[MATH_MULTILINE_STATE] = mathEndRules(MATH_MULTILINE_CLOSING_REGEX);
      this.$rules[MATH_ALTERNATIVE_STATE] = mathEndRules(MATH_ALTERNATIVE_CLOSING_REGEX);
