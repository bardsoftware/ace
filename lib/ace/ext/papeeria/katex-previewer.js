...
(function() {
  ...
  define(function(require, exports, module) {
    var ConstrainedTokenIterator, ContextHandler, EquationRangeHandler, LatexParsingContext, Range, TokenIterator, TokenSequenceFinder, findSurroundingBrackets, initKaTeX, katex, myKatexLoader;
    ...
    TokenSequenceFinder = (function() {
      function TokenSequenceFinder(tokenSequences, equalTokens) {
        this.tokenSequences = tokenSequences;
        this.equalTokens = equalTokens;
        this.nSequences = this.tokenSequences.length;
        this.currentIndices = Array(this.nSequences);
        this.currentIndices.fill(0);
      }

      TokenSequenceFinder.prototype.progressIndices = function(token) {
        var currentIndex, i, j, ref, results, tokenSequence;
        results = [];
        for (i = j = 0, ref = this.nSequences - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
          tokenSequence = this.tokenSequences[i];
          currentIndex = this.currentIndices[i];
          if (this.equalTokens(token, tokenSequence[currentIndex])) {
    ...
    EquationRangeHandler = (function() {
      ...
      EquationRangeHandler.prototype.getBoundary = function(tokenIterator, start) {
        var boundarySequence, boundarySequences, curToken, curTokenColumn, curTokenLength, curTokenRow, equationInnerBoundary, equationOuterBoundary, finishedSequence, finishedSequenceStringLength, i, j, k, l, len, len1, maybeFinishedSequenceId, moveFromBoundary, moveToBoundary, ref, ref1, ref2, token, tokenSequenceFinder;
        ...
        while (true) {
          ...
          if (maybeFinishedSequenceId != null) {
            ...
            finishedSequenceStringLength = ((function() {
              var l, len2, results;
              results = [];
              for (l = 0, len2 = finishedSequence.length; l < len2; l++) {
                token = finishedSequence[l];
            ...
            equationInnerBoundary = {
              row: curTokenRow,
              column: curTokenColumn + (start ? finishedSequenceStringLength : curTokenLength - finishedSequenceStringLength)
            };
            for (i = l = 0, ref2 = finishedSequence.length; 0 <= ref2 ? l <= ref2 : l >= ref2; i = 0 <= ref2 ? ++l : --l) {
              moveFromBoundary();
            }
