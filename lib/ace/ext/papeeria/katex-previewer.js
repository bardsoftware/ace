...
(function() {
  define(function(require, exports, module) {
    var EquationRangeHandler, LatexParsingContext, Range, TokenIterator, findSurroundingBrackets;
    ...
    EquationRangeHandler = (function() {
      ...
      EquationRangeHandler.prototype.equalTokens = function(token1, token2) {
      ...
      EquationRangeHandler.prototype.getEquationStart = function(tokenIterator) {
        ...
        ref = this.constructor.BEGIN_EQUATION_TOKEN_SEQUENCE;
        for (j = 0, len = ref.length; j < len; j++) {
          token = ref[j];
          if (this.equalTokens(token, tokenIterator.getCurrentToken())) {
            tokenIterator.stepForward();
          }
        }
        curSequenceIndex = this.constructor.BEGIN_EQUATION_TOKEN_SEQUENCE.length - 1;
        curEquationStart = null;
        while (curSequenceIndex >= 0) {
          if (this.equalTokens(this.constructor.BEGIN_EQUATION_TOKEN_SEQUENCE[curSequenceIndex], tokenIterator.stepBackward())) {
            if (curSequenceIndex === this.constructor.BEGIN_EQUATION_TOKEN_SEQUENCE.length - 1) {
      ...
      EquationRangeHandler.prototype.getEquationEnd = function(tokenIterator) {
        ...
        ref = this.constructor.END_EQUATION_TOKEN_SEQUENCE.slice(0).reverse();
        for (j = 0, len = ref.length; j < len; j++) {
          token = ref[j];
          if (this.equalTokens(token, tokenIterator.getCurrentToken())) {
        ...
        curSequenceIndex = 0;
        curEquationStart = null;
        while (curSequenceIndex < this.constructor.END_EQUATION_TOKEN_SEQUENCE.length) {
          if (this.equalTokens(this.constructor.END_EQUATION_TOKEN_SEQUENCE[curSequenceIndex], tokenIterator.stepForward())) {
    ...
    exports.setupPreviewer = function(editor, popoverHandler) {
      ...
      ch = ContextHandler = {
        ...
        getWholeEquation: function(range) {
          var argumentRange, bracketPosition, curColumn, curLabelParameter, curLabelTokens, curLine, curRow, labelParameters, labelSequenceIndex, noPush, ref, ref1, session, token, tokenIterator, tokenPosition, tokenValues;
          tokenValues = [];
