...
(function() {
  ...
  define(function(require, exports, module) {
    var ConstrainedTokenIterator, ContextHandler, EquationRangeHandler, LatexParsingContext, Range, TokenIterator, findSurroundingBrackets, initKaTeX, katex, myKatexLoader;
    ...
    ContextHandler = (function() {
      ...
      ContextHandler.prototype.getCurrentFormula = function() {
        var e, endString, equationString, error, labelParameters, ref, ref1, startColumn, startRow, startString, title, tokenIterator;
        try {
          if (this.curStartId !== this.curEndId) {
            startString = EquationRangeHandler.BEGIN_EQUATION_TOKEN_SEQUENCES[this.curStartId].slice(0).reverse().join("");
            endString = EquationRangeHandler.END_EQUATION_TOKEN_SEQUENCES[this.curEndId].join("");
      ...
      ContextHandler.prototype.updateRange = function() {
        ...
        return ref1 = this.equationRangeHandler.getEquationRange(cursorRow, cursorColumn), this.curOuterRange = ref1[0], this.curInnerRange = ref1[1], this.curStartId = ref1[2], this.curEndId = ref1[3], ref1;
      };
    ...
    EquationRangeHandler = (function() {
      ...
      EquationRangeHandler.prototype.getBoundary = function(tokenIterator, start) {
        var allowance, allowedSequences, anyAllowed, boundarySequence, boundarySequences, curEquationInnerBoundary, curEquationOuterBoundary, curSequence, curSequenceId, curSequenceIndex, curToken, curTokenPosition, foundSequence, i, j, k, l, len, len1, len2, m, moveFromBoundary, moveToBoundary, n, o, ref, ref1, ref2, ref3, token;
        ...
        while (true) {
          ...
          if (curSequence !== null) {
            if (EquationRangeHandler.equalTokens(curSequence[curSequenceIndex], curToken)) {
              ...
              anyAllowed = false;
              for (l = 0, len2 = allowedSequences.length; l < len2; l++) {
                allowance = allowedSequences[l];
                anyAllowed = anyAllowed || allowance;
              }
              if (!anyAllowed) {
                allowedSequences.fill(true);
              } else {
                for (i = m = 0, ref1 = curSequenceIndex - 1; 0 <= ref1 ? m <= ref1 : m >= ref1; i = 0 <= ref1 ? ++m : --m) {
                  moveFromBoundary();
                }
              }
              curSequence = null;
          ...
          if (curSequence === null) {
            foundSequence = false;
            curToken = tokenIterator.getCurrentToken();
            for (i = n = 0, ref2 = boundarySequences.length - 1; 0 <= ref2 ? n <= ref2 : n >= ref2; i = 0 <= ref2 ? ++n : --n) {
              boundarySequence = boundarySequences[i];
              if (allowedSequences[i] && EquationRangeHandler.equalTokens(boundarySequence[0], curToken)) {
          ...
          if ((curSequence != null) && curSequenceIndex >= curSequence.length) {
            curTokenPosition = tokenIterator.getCurrentTokenPosition();
            curEquationOuterBoundary = {
              row: curTokenPosition.row,
              column: curTokenPosition.column + (start ? 0 : curToken.value.length)
            };
            break;
          }
        }
        for (i = o = 0, ref3 = curSequence.length; 0 <= ref3 ? o <= ref3 : o >= ref3; i = 0 <= ref3 ? ++o : --o) {
          moveFromBoundary();
        }
      ...
      EquationRangeHandler.prototype.getEquationRange = function(row, column) {
        ...
        endId = end[0], outerEnd = end[1], innerEnd = end[2];
        startId = start[0], outerStart = start[1], innerStart = start[2];
