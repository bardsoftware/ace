...
define(function(require, exports, module) {
    ...
    module.exports = {
        ...
        "test: ContextHandler.getFormula(), mismatching boundaries": function() {
            var session = new EditSession(["\\begin{equation} \\text{hi there!} $$"]);
            //                               01234567890123456 78901234 56789012345678901 234567890123456
            //                               0         10         20         30        40         50
            var editor = new Editor(new MockRenderer(), session);
            session.setMode("./mode/papeeria_latex");

            var popoverHandler = new MockPopoverHandler;
            var contextHandler = new ContextHandler(editor, null, popoverHandler, new EquationRangeHandler(editor));
            editor.moveCursorTo(0, 20);
            contextHandler.updateRange();
            var currentFormula = contextHandler.getCurrentFormula();

            assert.equal("Error!", currentFormula.title);
            assert.equal("Starting and ending sequences don't match: \\begin{equation} and $$", currentFormula.content);
        },
