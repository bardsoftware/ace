...
define(function(require, exports, module) {
    ...
    module.exports = {
        ...
        "test: EquationRangeHandler, ending with empty line with some text after": function() {
            ...
            for (var state in mathConstants) {
                var stateConstants = mathConstants[state];
                var session = new EditSession([stateConstants.start + content, "", "stuff" + stateConstants.end]);
                var editor = new Editor(new MockRenderer(), session);
                session.setMode("./mode/papeeria_latex");
                ...
                assert(!equationRange.correct);
                assert.range(equationRange.range, 0, equationStart, 2, 5);
            }
        },
        ...
        "test: EquationRangeHandler, comments": function() {
            var session = new EditSession([
                "\\begin{equation}hi %a comment",
            ...
            var equationRange = new EquationRangeHandler(editor).getEquationRange(0, 17);

            assert(equationRange.correct);
            assert.range(equationRange.range, 0, 16, 2, 0);
        },
        ...
        "test: ContextHandler.extractEquation, comments": function() {
            var session = new EditSession([
                "\\begin{equation}hi %a comment",
            //    012345678901234567890123456789
            //    0         1         2
            ...
            var range = new Range(0, 16, 2, 0);

            var labelsAndEquation = ContextHandler.extractEquation(session, range);
            assert.equal(labelsAndEquation.equation, "hi hey ");
        }
    }
});
