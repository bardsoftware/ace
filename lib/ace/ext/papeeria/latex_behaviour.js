...
(function() {
  ...
  define(function(require, exports, module) {
    var AUTO_INSERT, Behaviour, COMMENT_TOKENTYPE, CORRESPONDING_CLOSING, DO_NOTHING, EQUATION_TOKENTYPE, ESCAPE_TOKENTYPE, KEYWORD_TOKENTYPE, LatexBehaviour, LatexParsingContext, PapeeriaLatexHighlightRules, RPAREN_TOKENTYPE, SKIP, SKIP_TWO, STORAGE_TOKENTYPE, dollarsDeletionAction, dollarsInsertionAction, getBracketInsertionAction, getBracketsDeletionAction, getContext, getWrapped, isEscapedInsertion, isType;
    ...
    COMMENT_TOKENTYPE = PapeeriaLatexHighlightRules.COMMENT_TOKENTYPE, ESCAPE_TOKENTYPE = PapeeriaLatexHighlightRules.ESCAPE_TOKENTYPE, RPAREN_TOKENTYPE = PapeeriaLatexHighlightRules.RPAREN_TOKENTYPE, EQUATION_TOKENTYPE = PapeeriaLatexHighlightRules.EQUATION_TOKENTYPE, STORAGE_TOKENTYPE = PapeeriaLatexHighlightRules.STORAGE_TOKENTYPE, KEYWORD_TOKENTYPE = PapeeriaLatexHighlightRules.KEYWORD_TOKENTYPE;
    getContext = LatexParsingContext.getContext, isType = LatexParsingContext.isType;
    ...
    dollarsInsertionAction = function(state, action, editor, session, text) {
      ...
      if (getContext(session, row, column) === COMMENT_TOKENTYPE || isEscapedInsertion(token, column)) {
        return DO_NOTHING;
      }
        ...
        switch (text) {
          case opening:
            ...
            if (isEscapedInsertion(token, column)) {
              shouldComplete = opening !== '{' && getContext(session, row, column) !== EQUATION_TOKENTYPE;
              if (shouldComplete) {
                return {
            ...
            if (!((ref1 = editor.completer) != null ? ref1.activated : void 0) && getContext(session, row, column) !== COMMENT_TOKENTYPE) {
              return {
                text: opening + closing,
                selection: [1, 1]
              };
